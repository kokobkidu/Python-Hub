...
@app.route('/')
def home():
    # ተጠቃሚው የመረጠው ቀን (ካልተመረጠ የዛሬው ቀን ይሆናል)
    selected_date = request.args.get('date', datetime.now().strftime('%Y-%m-%d'))
    
    # አናት ላይ የሚታዩትን ያለፉትን እና የሚመጡትን 7 ቀናት ማዘጋጀት (እንደ Besoccer)
    dates_list = []
    base_date = datetime.now() - timedelta(days=2)
    for i in range(7):
        d = base_date + timedelta(days=i)
        d_str = d.strftime('%Y-%m-%d')
        d_label = d.strftime('%a %d %b') # ምሳሌ፦ Sun 19 Jul
        dates_list.append({'date': d_str, 'label': d_label})
    
    # የኤፒአይ ጥያቄ በተመረጠው ቀን መሰረት (ሁሉንም ሊጎች እና ጨዋታዎች እንዲያመጣ)
    ...
    <head>
        ...
        <style>
            ...
            /* Besoccer style dates scrollable bar */
            ...
            .league-title {{ font-size: 12px; font-weight: bold; color: #444; margin: 15px 5px 6px 5px; text-transform: uppercase; background: #e0e0e0; padding: 5px 8px; border-radius: 4px; }}
    ...
    <body>
        ...
        <!-- ቀኖቹን በሰንጠረዥ መልክ ማሳያ (Besoccer style) -->
        <div class="date-tabs">
    """
    ...
    try:
        ...
        if matches:
            ...
            for match in matches:
                ...
                # ሊጎቹን እና የጨዋታ አይነቶችን (Friendly, Women, ወዘተ) በግልጽ ማሳየት
                ...
                match_time = match['fixture']['date'][11:16] # የሰዓት መረጃ (HH:MM)
